...
#自动选择下载节点
get_node_url(){
	nodes=(https://raw.githubusercontent.com/vipfan/BtPanel6.x/master http://125.88.182.172:5880 http://103.224.251.67 http://128.1.164.196 http://download.bt.cn );
	i=1;
	if [ ! -f /bin/curl ];then
	...
	if [ $j = 5000 ];then
		NODE_URL='https://raw.githubusercontent.com/vipfan/BtPanel6.x/master';
	else
		NODE_URL=${urls[$j]}
	fi
